...
public class Base64 {
	...
	public static byte[] decodeFromFile(String filename) {
		...
		try {
			...
			if (file.length() > Integer.MAX_VALUE) {
				System.err.println("File is too big for this convenience method (" + file.length()
				        + " bytes).");
				return null;
	...
	public static Object decodeToObject(String encodedObject) {
		return Base64.decodeToObject(encodedObject, -1);
	}
	...
	public static Object decodeToObject(String encodedObject, int options) {
		...
		byte[] objBytes = null;
		if (options != -1) {
			objBytes = Base64.decode(encodedObject, options);
		} else {
			objBytes = Base64.decode(encodedObject);
		}
	...
	/**
	 * Encodes or decodes two files from the command line; <strong>feel free to delete this method
	 * (in fact you probably should) if you're embedding this code into a larger program.</strong>
	 */
